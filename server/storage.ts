...
export class DatabaseStorage implements IStorage {
  ...
  async createStressAnalysis(analysis: InsertStressAnalysis): Promise<StressAnalysis> {
    const preparedAnalysis = {
      ...analysis,
      recommendedActions:
        typeof analysis.recommendedActions === "string"
          ? [analysis.recommendedActions]
          : analysis.recommendedActions,
      triggerFactors:
        typeof analysis.triggerFactors === "string"
  ...
  async getAlertHistory(userId: string): Promise<AlertHistoryRecord[]> {
    return await db
      .select()
      .from(alertHistory)
  ...
  async getChildProfile(childUserId: string): Promise<any> {
    const [user] = await db.select().from(users).where(eq(users.id, childUserId));
    return user || null;
  }
...
class MemoryStorage implements IStorage {
  ...
  private alertHistoryMap = new Map<string, AlertHistoryEntry[]>();
  ...
  private persistentDataPath = 'persistent-data.json';
    ...
    try {
      ...
      if (fs.existsSync(dataPath)) {
        ...
        if (data.emergencyContacts) {
          for (const [userId, contacts] of Object.entries(data.emergencyContacts)) {
  ...
  private async savePersistentData() {
    try {
      ...
      const data = {
        ...
        emergencyContacts: Object.fromEntries(this.emergencyContactsMap),
  ...
  async createEmergencyContact(contact: InsertEmergencyContact): Promise<EmergencyContact> {
    ...
    const newContact = { ...contact, id: Date.now(), createdAt: new Date() } as EmergencyContact;
  ...
  // Safe zones operations
  private safeZonesMap = new Map<string, SafeZone[]>();

  async getSafeZones(userId: string): Promise<SafeZone[]> {
    return this.safeZonesMap.get(userId) || [];
  }

  async createSafeZone(zone: InsertSafeZone): Promise<SafeZone> {
    const userId = zone.userId ?? "demo-user";
    const zones = this.safeZonesMap.get(userId) || [];
    ...
    zones.push(newZone);
    this.safeZonesMap.set(userId, zones);
    return newZone;
  }

  async deleteSafeZone(id: number): Promise<boolean> {
    for (const [userId, zones] of this.safeZonesMap) {
  ...
  async setHomeLocation(homeLocation: InsertHomeLocation): Promise<HomeLocation> {
    const record: HomeLocation = {
      id: Date.now(),
      userId: homeLocation.userId,
      latitude: homeLocation.latitude,
      longitude: homeLocation.longitude,
    ...
    this.homeLocationsMap.set(homeLocation.userId, record);
    return record;
  }

  async updateHomeLocation(userId: string, updates: Partial<InsertHomeLocation>): Promise<HomeLocation | undefined> {
    const existing = this.homeLocationsMap.get(userId);
    if (!existing) return undefined;
    const updated: HomeLocation = { ...existing, ...updates, updatedAt: new Date() };
    this.homeLocationsMap.set(userId, updated);
    return updated;
  }
    ...
    try {
      ...
      if (storedHistory) {
        for (const [userId, alerts] of Object.entries(storedHistory)) {
          this.alertHistoryMap.set(userId, alerts as AlertHistoryEntry[]);
        }
      }
    } catch (error) {
  ...
  async archiveResolvedAlert(alertId: number, resolvedBy: string): Promise<void> {
    ...
    if (alert) {
      ...
      const createdAt = alert.createdAt ?? new Date();
      const historyEntry = {
        id: Date.now(),
        ...
        responseTime: Math.floor((Date.now() - new Date(createdAt).getTime()) / 1000),
        audioRecordingUrl: alert.audioRecordingUrl,
        videoRecordingUrl: alert.videoRecordingUrl,
        isArchived: false,
        createdAt,
  ...
  async createFamilyConnection(connection: InsertFamilyConnection): Promise<FamilyConnection> { 
    const newConnection: FamilyConnection = {
      ...
      relationshipType: connection.relationshipType ?? "parent-child",
...
class SmartStorage implements IStorage {
  ...
  async getSafeZones(userId: string): Promise<SafeZone[]> {
    try {
      ...
      return this.memStorage.getSafeZones(userId);
    }
  }
  ...
  async createSafeZone(zone: InsertSafeZone): Promise<SafeZone> {
    try {
      ...
      return this.memStorage.createSafeZone(zone);
    }
  }
  ...
  async deleteSafeZone(id: number): Promise<boolean> {
    try {
      ...
      return this.memStorage.deleteSafeZone(id);
    }
  }
  ...
  async deleteDestination(id: number): Promise<boolean> {
    try {
      ...
      return this.memStorage.deleteDestination(id);
    }
  }
  ...
  async setHomeLocation(homeLocation: InsertHomeLocation): Promise<HomeLocation> {
    try {
      ...
      return this.memStorage.setHomeLocation(homeLocation);
    }
  }
  ...
  async updateHomeLocation(userId: string, updates: Partial<InsertHomeLocation>): Promise<HomeLocation | undefined> {
    try {
      ...
      return this.memStorage.updateHomeLocation(userId, updates);
    }
  }
  ...
  async createOtpVerification(otp: InsertOtpVerification): Promise<OtpVerification> {
    try {
      ...
      return this.memStorage.createOtpVerification(otp);
    }
  }
  ...
  async verifyOtp(identifier: string, type: string, otp: string): Promise<boolean> {
    try {
      ...
      return this.memStorage.verifyOtp(identifier, type, otp);
    }
  }
  ...
  async getHealthMetrics(userId: string, limit?: number): Promise<HealthMetric[]> {
    try {
      ...
      return this.memStorage.getHealthMetrics(userId, limit);
    }
  }
  ...
  async createHealthMetric(metric: InsertHealthMetric): Promise<HealthMetric> {
    try {
      ...
      return this.memStorage.createHealthMetric(metric);
    }
  }
  ...
  async getLatestHealthMetrics(userId: string): Promise<HealthMetric | undefined> {
    try {
      ...
      return this.memStorage.getLatestHealthMetrics(userId);
    }
  }
  ...
  async getStressAnalysis(userId: string, limit?: number): Promise<StressAnalysis[]> {
    try {
      ...
      return this.memStorage.getStressAnalysis(userId, limit);
    }
  }
  ...
  async createStressAnalysis(analysis: InsertStressAnalysis): Promise<StressAnalysis> {
    try {
      ...
      return this.memStorage.createStressAnalysis(analysis);
    }
  }
  ...
  async getLatestStressAnalysis(userId: string): Promise<StressAnalysis | undefined> {
    try {
      ...
      return this.memStorage.getLatestStressAnalysis(userId);
    }
  }
  ...
  async getIotEmergencyTriggers(userId: string): Promise<IotEmergencyTrigger[]> {
    try {
      ...
      return this.memStorage.getIotEmergencyTriggers(userId);
    }
  }
  ...
  async createIotEmergencyTrigger(trigger: InsertIotEmergencyTrigger): Promise<IotEmergencyTrigger> {
    try {
      ...
      return this.memStorage.createIotEmergencyTrigger(trigger);
    }
  }
  ...
  async resolveIotEmergencyTrigger(id: number): Promise<boolean> {
    try {
      ...
      return this.memStorage.resolveIotEmergencyTrigger(id);
    }
  }
