...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  const placesCache = new Map<string, { expiresAt: number; payload: any }>();
  const getCachedPlaces = (key: string) => {
  ...
  const setCachedPlaces = (key: string, payload: any, ttlMs = 5 * 60 * 1000) => {
    placesCache.set(key, { expiresAt: Date.now() + ttlMs, payload });
  };
      ...
      const buildMockResults = () => ({
        ...
        results: [
          ...
          },
          {
            ...
            name: "City Hospital",
            geometry: { location: { lat: latitude - 0.008, lng: longitude + 0.006 } },
            vicinity: "Health Avenue",
            types: ["hospital"]
          },
          {
            place_id: "mock-metro-1",
            name: "Central Metro Station",
            geometry: { location: { lat: latitude + 0.004, lng: longitude - 0.007 } },
            vicinity: "Transit Road",
            types: ["subway_station"]
          }
        ]
      });

      const buildOsmResults = async () => {
        const cacheKey = `nearby:${type}:${lat}:${lng}:${radius}`;
        const cached = getCachedPlaces(cacheKey);
        if (cached) {
          return cached;
        }

        const osmTypeMapping: Record<string, Record<string, string>> = {
        ...
        const tagFilters = osmTypeMapping[normalizedType];

        const buildTagFilter = (tags: Record<string, string>) =>
          Object.entries(tags)
            .map(([key, value]) => `["${key}"="${value}"]`)
            .join('');

        const radiusMeters = Math.min(Math.max(parseInt(radius as string, 10) || 5000, 200), 20000);

        const queryFilters = tagFilters
          ? buildTagFilter(tagFilters)
          : `["name"~"${normalizedType.replace(/"/g, '')}",i]`;

        const overpassQuery = `
[out:json][timeout:25];
(
  node(around:${radiusMeters},${latitude},${longitude})${queryFilters};
  way(around:${radiusMeters},${latitude},${longitude})${queryFilters};
  relation(around:${radiusMeters},${latitude},${longitude})${queryFilters};
);
out center;
        `.trim();
          ...
          try {
            const response = await fetchWithTimeout(endpoint, {
              ...
              headers: {
                ...
                'User-Agent': 'SakhiSuraksha/1.0 (local-development)',
                'Accept-Language': 'en'
              },
        ...
        const results = (data.elements || []).map((element: any) => {
          ...
          const name = tags.name || tags.operator || tags.amenity || tags.shop || tags.railway || 'Nearby Place';
          const vicinityParts = [
            tags['addr:street'],
            tags['addr:suburb'],
            tags['addr:city'],
          ...
          return {
            ...
            name,
            geometry: { location: { lat: latValue, lng: lngValue } },
            vicinity: vicinityParts.join(', '),
            types: Object.values(tags).slice(0, 4),
          };
        }).filter((item: any) => item.geometry.location.lat && item.geometry.location.lng);

        const payload = { status: 'OK', provider: 'osm', results };
        setCachedPlaces(cacheKey, payload);
        return payload;
      };

      if (!apiKey) {
        try {
          return res.json(await buildOsmResults());
        } catch (osmError) {
          console.error('OSM Overpass error:', osmError);
          return res.json(buildMockResults());
        }
      }
      ...
      if (data.status === 'OK') {
        ...
        try {
          res.json(await buildOsmResults());
        } catch (osmError) {
          console.error('OSM Overpass error:', osmError);
          res.json(buildMockResults());
        }
      }
    } catch (error) {
...
[out:json][timeout:25];
(
  node(around:${radiusMeters},${latitude},${longitude})["name"~"${normalizedType.replace(/"/g, '')}",i];
  way(around:${radiusMeters},${latitude},${longitude})["name"~"${normalizedType.replace(/"/g, '')}",i];
  relation(around:${radiusMeters},${latitude},${longitude})["name"~"${normalizedType.replace(/"/g, '')}",i];
);
out center;`.trim();
          ...
          for (const endpoint of overpassEndpoints) {
            try {
              ...
              console.error('OSM Overpass fallback error:', error);
            }
          }
          ...
          if (data) {
            const results = (data.elements || []).map((element: any) => {
              ...
              return {
                ...
                name,
                geometry: { location: { lat: latValue, lng: lngValue } },
                vicinity: tags['addr:street'] || tags['addr:city'] || '',
                types: Object.values(tags).slice(0, 4),
              };
            }).filter((item: any) => item.geometry.location.lat && item.geometry.location.lng);
            return res.json({ status: 'OK', provider: 'osm', results });
          }
        }
      } catch (osmError) {
        console.error('OSM Overpass fallback error:', osmError);
      }
  ...
  app.get("/api/places/search", async (req, res) => {
    try {
      ...
      const buildOsmSearchResults = async () => {
        ...
        const searchUrl = `https://nominatim.openstreetmap.org/search?format=jsonv2&limit=10&addressdetails=1&q=${encodeURIComponent(query as string)}`;
      ...
      if (!apiKey) {
        try {
          return res.json(await buildOsmSearchResults());
        } catch (osmError) {
      ...
      if (data.status === 'OK') {
        ...
        try {
          res.json(await buildOsmSearchResults());
        } catch (osmError) {
          console.error('OSM Nominatim error:', osmError);
          res.json(buildMockResults());
        }
      ...
      try {
        ...
        console.error('OSM Nominatim fallback error:', osmError);
      }

      res.json({ status: "OK", mock: true, results: [] });
    }
  });
  ...
  app.post("/api/voice/analyze", async (req, res) => {
    try {
      ...
      // Mock voice analysis response (replace with actual AI service)
      const analysis: {
        isEmergency: boolean;
        confidence: number;
        keywords: string[];
      ...
      // If keywords indicate distress, mark as emergency
      ...
      if (detectedKeywords.length > 0) {
        ...
        if (analysis.confidence > 0.6 && userId) {
          await storage.createEmergencyAlert({
            ...
            latitude: undefined,
            longitude: undefined,
            address: 'Voice-triggered emergency'
          });
        }
      }
  ...
  // Nearby places API for maps integration
  app.get("/api/nearby/safe-places", async (req, res) => {
    try {
      ...
      // Mock safe places data (replace with actual Google Places API or similar)
      const safePlaces = [
        {
          id: 'police-1',
          name: 'City Police Station',
          type: 'police_station',
          latitude: parseFloat(latitude as string) + 0.01,
          longitude: parseFloat(longitude as string) + 0.01,
          distance: 1200,
        ...
        },
        {
          ...
          name: '24/7 Pharmacy',
          type: 'pharmacy',
          latitude: parseFloat(latitude as string) + 0.005,
          longitude: parseFloat(longitude as string) - 0.005,
          distance: 500,
  ...
  app.get("/api/nearby/community-alerts", async (req, res) => {
    try {
      ...
      const alerts = await storage.getCommunityAlerts(
      ...
      res.status(500).json({ message: "Failed to fetch community alerts" });
    }
  });
  ...
  async function triggerEmergencyProtocol(alert: any) {
    try {
      ...
      message += `Please check on Pranav's safety immediately.`;
        ...
        for (const contact of contacts) {
          try {
            ...
            if (contact.phoneNumber) {
              whatsappSuccess = await sendWhatsAppEmergency(contact.phoneNumber, message);
              console.log(`WhatsApp to ${contact.name}: ${whatsappSuccess ? 'SUCCESS' : 'FAILED'}`);

              smsSuccess = await sendSMSEmergency(
                contact.phoneNumber,
                { address: locationText, lat: alert.latitude ?? undefined, lng: alert.longitude ?? undefined },
                message
              );
              console.log(`SMS to ${contact.name}: ${smsSuccess ? 'SUCCESS' : 'FAILED'}`);
            }
      ...
      const children = await Promise.all(connections.map(async (connection: any) => {
        ...
        return {
          ...
          currentLocation: homeLocation ? {
            lat: Number(homeLocation.latitude),
            lng: Number(homeLocation.longitude),
  ...
  app.get("/api/emergency-alerts/:id", async (req, res) => {
    try {
      ...
      if (alert.audioRecordingUrl) {
        try {
          const parsed = JSON.parse(alert.audioRecordingUrl);
          detectedText = parsed?.detectedText || parsed?.scenario || "Emergency detected";
        } catch {
          detectedText = alert.audioRecordingUrl;
        }
      }
      ...
      const formattedAlert = {
        ...
        voiceDetectionText: detectedText
      };
    ...
    try {
      ...
      childAlerts.forEach((alert: EmergencyAlert) => {
        try {
          ...
          const videoUrl = alert.videoRecordingUrl && alert.videoRecordingUrl.startsWith('/api/emergency-recordings/')
            ? alert.videoRecordingUrl
            : null;
          ...
          const alertData = {
            ...
            message: getAlertMessage(alert.triggerType, location, timestampStr, alert.audioRecordingUrl ?? undefined),
            location,
  ...
  app.get("/api/emergency-recordings/:streamId/:filename", async (req, res) => {
    try {
      ...
      const filePath = path.join(recordingsDir, streamId, filename);
      let resolvedPath = path.resolve(filePath);

      // Prevent path traversal
      if (!resolvedPath.startsWith(path.resolve(recordingsDir))) {
        return res.status(400).json({ message: "Invalid recording path" });
      }

      if (!fs.existsSync(resolvedPath)) {
        // Legacy fallback path: server/<streamId>/<filename>
        const legacyDir = path.join(process.cwd(), 'server');
        const legacyPath = path.resolve(path.join(legacyDir, streamId, filename));
        if (!legacyPath.startsWith(path.resolve(legacyDir)) || !fs.existsSync(legacyPath)) {
  ...
  app.get("/api/parent/live-location/:childId", async (req, res) => {
    try {
      ...
      const currentLocation = {
        ...
        lng: homeLocation ? Number(homeLocation.longitude) + (Math.random() - 0.5) * 0.001 : 77.5946,
...
Please respond immediately if you can assist.`;
      ...
      if (phoneNumber) {
        try {
          ...
          const smsResult = await sendSMSEmergency(
            phoneNumber,
            emergencyData.location.address ? emergencyData.location.address : undefined,
      ...
      if (email) {
        try {
          emailSuccess = await sendEmailAlert(email, enhancedMessage);
  ...
  // Test SMS, WhatsApp, and Email services
  app.post('/api/test-services', async (req, res) => {
    try {
      const { phoneNumber, email } = req.body;

      const testMessage = "Sakhi Suraksha test: SMS connectivity check.";

      let smsResult: { success: boolean; error: string | null } = { success: false, error: null };
      let whatsappResult: { success: boolean; error: string | null } = { success: false, error: null };
      let emailResult: { success: boolean; error: string | null } = { success: false, error: null };

      // Test Twilio SMS if phone number provided
      if (phoneNumber) {
        try {
          console.log(`Test services: sending SMS to ${phoneNumber}`);
          smsResult.success = await sendSMS(phoneNumber, testMessage);
          if (!smsResult.success && !smsResult.error) {
            smsResult.error = 'SMS send failed (check server logs for Twilio response)';
          }
          console.log(`Test services: SMS result for ${phoneNumber}: ${smsResult.success ? 'SUCCESS' : 'FAILED'}`);
        } catch (error) {
          const err = error instanceof Error ? error : new Error(String(error));
          smsResult.error = err.message;
          console.error(`Test services: SMS error for ${phoneNumber}:`, err);
        }
      }

      // Test WhatsApp OTP if phone number provided
      if (phoneNumber) {
        try {
          console.log(`Test services: sending WhatsApp OTP to ${phoneNumber}`);
          whatsappResult.success = await sendWhatsAppOTP(phoneNumber, "123456");
          if (!whatsappResult.success && !whatsappResult.error) {
            whatsappResult.error = 'WhatsApp send failed (check server logs for API response)';
          }
          console.log(`Test services: WhatsApp result for ${phoneNumber}: ${whatsappResult.success ? 'SUCCESS' : 'FAILED'}`);
        } catch (error) {
          const err = error instanceof Error ? error : new Error(String(error));
          whatsappResult.error = err.message;
          console.error(`Test services: WhatsApp error for ${phoneNumber}:`, err);
        }
      }
      ...
      if (email) {
        try {
          emailResult.success = await sendEmailAlert(email, testMessage);
        } catch (error) {
          const err = error instanceof Error ? error : new Error(String(error));
      ...
      res.status(500).json({ message: 'Service test failed', error: err.message });
    }
  });
  ...
  app.get('/api/live-stream/:streamId', async (req, res) => {
    try {
      ...
      if (streamParam.startsWith('stream_')) {
        ...
        stream = streams.find((s: LiveStream) => s.shareLink?.includes(streamParam));
      } else {
        // For numeric IDs
  ...
  app.post('/api/live-stream/start', async (req, res) => {
    try {
      const { streamUrl, shareableLink, isEmergency, latitude, longitude, address, triggerType, scenario, detectedText, alertId } = req.body;
      ...
      if (isEmergency && alertId) {
        emergencyAlertId = Number(alertId);
      } else if (isEmergency) {
      ...
      res.json({
        success: true,
        streamId: stream.id,
        shareableLink: responseShareableLink,
  ...
  app.post('/api/emergency/save-session-recording', upload.single('video'), async (req: any, res) => {
    try {
      const { sessionType, userId, timestamp, alertId } = req.body;
      const videoFile = req.file;
      ...
      const recordingsDir = path.join(process.cwd(), 'server', 'emergency-recordings');
      const folderName = alertId ? `emergency_${alertId}` : `session_${Date.now()}`;
      ...
      const videoUrl = `/api/emergency-recordings/${folderName}/${fileName}`;
      ...
      console.log(`Emergency session recording received: ${sessionType} for user ${userId}. Saved to ${videoUrl}`);
