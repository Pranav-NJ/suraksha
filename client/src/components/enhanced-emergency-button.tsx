...
interface ActiveEmergencyAlert {
  id: number;
...
export default function EnhancedEmergencyButton() {
  ...
  const { data: activeAlerts = [] } = useQuery<ActiveEmergencyAlert[]>({
  ...
  useEffect(() => {
    const hasActiveAlert = activeAlerts.some((alert: ActiveEmergencyAlert) => !alert.isResolved);
    if (hasActiveAlert && !emergencyActive) {
      const activeAlert = activeAlerts.find((alert: ActiveEmergencyAlert) => !alert.isResolved);
  ...
  // Persist current alert id for streaming room matching
  useEffect(() => {
    if (currentAlertId) {
      sessionStorage.setItem('currentEmergencyAlertId', String(currentAlertId));
    } else {
