...
export default function FamilyConnectionQR() {
  ...
  const { data: connections = [] } = useQuery<FamilyConnection[]>({
                ...
                <div key={connection.id} className="flex items-center justify-between p-3 border rounded-lg">
                  <div>
                    ...
                    <div className="text-sm text-gray-500">
                      {connection.relationshipType} • Connected {connection.acceptedAt ? new Date(connection.acceptedAt).toLocaleDateString() : "Pending"}
                    </div>
                  </div>
